...
serve(async (request) => {
  ...
  try {
    ...
    const claimResult = await client.rpc("claim_ad_reward_action", {
      p_cooldown_minutes: serverConfig.adRewardCooldownMinutes,
      p_key_reward: serverConfig.adRewardKeys,
      p_user_id: userId
    });
    ...
    if (claimResult.error || !reward) {
      throw new Error(claimResult.error?.message ?? "Unable to claim ad reward.");
    }
