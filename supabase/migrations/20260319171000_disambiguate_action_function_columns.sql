...
begin
  ...
  if v_opened_cells_count >= 8 then
    ...
    update public.users as u
    set coins = u.coins + v_next_coins,
        shards = u.shards + v_next_shards,
        total_coins_earned = u.total_coins_earned + v_next_coins,
        total_shards_earned = u.total_shards_earned + v_next_shards
    where u.id = p_user_id;
...
begin
  ...
  update public.users as u
  set coins = u.coins + v_raid.temporary_coins,
      shards = u.shards + v_raid.temporary_shards,
      total_coins_earned = u.total_coins_earned + v_raid.temporary_coins,
