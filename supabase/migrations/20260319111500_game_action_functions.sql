...
begin
  ...
  select count(*),
         count(*) filter (where cell_type = 'bomb'),
         count(*) filter (where cell_index < 0 or cell_index > 8),
         count(*) - count(distinct cell_index)
...
begin
  ...
  if v_opened_cells_count >= 8 then
    ...
    update public.users
    set coins = coins + v_next_coins,
        shards = shards + v_next_shards,
        total_coins_earned = total_coins_earned + v_next_coins,
        total_shards_earned = total_shards_earned + v_next_shards
    where id = p_user_id;

    v_credit_coins := v_next_coins;
    ...
    if v_credit_coins > 0 then
      ...
      values (
        ...
        'coins',
        v_credit_coins,
        jsonb_build_object('raid_id', p_raid_id, 'source', 'raid_auto_win')
      );
    end if;
...
begin
  ...
  update public.users
  set coins = coins + v_raid.temporary_coins,
      shards = shards + v_raid.temporary_shards,
      total_coins_earned = total_coins_earned + v_raid.temporary_coins,
      total_shards_earned = total_shards_earned + v_raid.temporary_shards
  where id = p_user_id;

  if v_raid.temporary_coins > 0 then
    insert into public.economy_logs (
      user_id,
      type,
      currency,
...
begin
  ...
  if v_user.last_daily_claim_at is not null
     and v_user.last_daily_claim_at + interval '24 hours' > v_now then
...
comment on function public.start_raid_session(uuid, jsonb) is
  'Atomically consumes 1 key, creates an active raid, and inserts the server-generated raid cells.';
comment on function public.reveal_raid_cell_action(uuid, uuid, integer, integer) is
  'Atomically reveals one raid cell, applies rewards, and resolves auto-win state if the player clears all safe cells.';
comment on function public.cash_out_raid_action(uuid, uuid, integer) is
  'Atomically cashes out an active raid and credits the earned coins and shards.';
comment on function public.claim_daily_reward_action(uuid) is
  'Atomically claims the next daily reward and writes the reward history log.';
